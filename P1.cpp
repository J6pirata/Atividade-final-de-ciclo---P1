...
int main(int argc, char** argv)
{
	...
	Data *hoje = new Data(timePtr->tm_mday, timePtr->tm_mon+1, timePtr->tm_year + 1900,0);
    cout << "Hoje eh " <<hoje->getData() << endl;
	cout<<endl;
    cout<<"*Cadastro de Contatos*"<< endl;
	
	Contato *contatos[CON];
	for(int i=0;i<CON;i++)
	{
	...
	contatos[i]->setIdade(hoje->getDia(),contatos[i]->getDia(),hoje->getMes(),contatos[i]->getMes(),hoje->getAno(),contatos[i]->getAno());
	cout<<contatos[i]->getNome()<<" nasceu em "<<contatos[i]->getData()<<" e tem " <<contatos[i]->getIdade()<<" ano(s) de idade."<<endl;
	
	somaidade=somaidade+contatos[i]->getIdade();
	
	if(contatos[i]->getIdade()>17)
	{
		maioridade++;
	}
	...
	cout<<endl<<"A media de idade dos "<<CON<<" contatos eh de "<<(media=somaidade/CON)<<" anos."<<endl;
	
	cout<<endl<<"Contato(s) com idade maior do que a media:"<<endl;
	for(int i=0;i<CON;i++)
	{
	...
	cout<<endl<<"No total tem "<<maioridade<<" pessoa(s) maior(es) de idade."<<endl;
	
	cout<<endl<<"Contato(s) mais velho(s) com "<<maisvelho<<" anos: "<<endl;
	for(int i=0;i<CON;i++)
	{
